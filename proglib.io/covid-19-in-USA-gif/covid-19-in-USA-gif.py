...
# Источник: сайт переписи населения census.gov:
# https://www.census.gov/data/tables/time-series/demo/popest/2010s-state-total.html
populations = pd.read_csv('nst-est2019-alldata.csv',
...
topfivestates_rate = list(df_0501.sort_values(
    by='rate', ascending=False).head()['state'])
# Добавим к ним также Вашингтон и Калифорнию,
# поскольку у них рано было зарегистрировано большое число случаев заболевания
...
# Ограничиваем начало диапазона 1 марта 2020
# так как до этой даты случаев заболевания было мало или информация о них неполная
...
# Создаем множество png-изображений для разных временных точек
# Затем мы просто сошьем их вместе, чтобы получить gif-изображение
plt.style.use('fivethirtyeight')
length = len(df.index)
for i in range(10, length+10):
    ax = df.iloc[:i].plot(figsize=(12, 8), linewidth=5, color=[
                          '#173F5F', '#20639B', '#2CAEA3', '#F6D55C', '#ED553B', '#B88BAC', '#827498'])
    ax.set_ylim(0, 2000)
    ax.set_xlabel('Количество дней после 1-го марта 2020 года')
    ax.set_ylabel('Количество случаев на 100,000 человек')
    ax.set_title(
        "Случаи заражения COVID-19 в США на 100,000 человек", fontsize=18)
